...
const MuscleMapSection = () => {
  ...
  return (
    <section id="features" className="bg-bg-secondary py-20 lg:py-32">
      <div className="max-w-[1200px] mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
        ...
        >
          ...
          {/* Toggle */}
          <div className="mt-6 flex bg-bg-tertiary rounded-pill p-1 border border-primary/[0.15]">
            <button
              onClick={() => setShowBack(false)}
              className={`px-6 py-2 rounded-pill text-sm font-medium transition-all duration-300 ${!showBack ? 'bg-primary text-foreground' : 'text-text-secondary'}`}
            >
              Front
            </button>
            <button
        ...
        >
          ...
          <motion.div variants={fadeUpVariant} className="mt-6 space-y-2">
            {features.map((f) => (
              <div key={f} className="flex items-start gap-2 text-text-secondary text-sm">
